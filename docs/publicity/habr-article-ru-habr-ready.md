...
Так появился `lay`: локальный помощник для GNOME Wayland, который исправляет
...
Проект ещё beta, но для моего ежедневного набора он уже стал рабочим
инструментом. Я делал его для себя, а теперь делюсь с миром: если кому-то тоже
не хватает лёгкого double Shift исправления слов под Linux, берите.
...
Сейчас `lay` — beta для GNOME Wayland и RU/EN раскладок.

Проверялось на Ubuntu/GNOME Wayland. Extension заявляет поддержку GNOME Shell
45, 46, 47 и 50.

KDE, Sway, Hyprland и другие окружения пока не цель первого релиза. Там нужна
другая интеграция.
...
- собрать обратную связь от GNOME Wayland пользователей.

Я не делал `lay` как большой продукт. Это маленькая утилита, которую я написал
под свою боль и выкладываю наружу: берите, если вам тоже нужно лёгкое
исправление слов по double Shift.
